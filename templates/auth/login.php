...
$inner = function () use ($e, $t, $csrf, $title, $sent, $error) {
  ...
  <?php if (!empty($sent)): ?>
    <p><?= $e($t('We sent a magic sign-in link to')) ?> <strong><?= $e($sent) ?></strong>. <?= $e($t('Open it on this device to continue.')) ?></p>
  <?php else: ?>
    <p style="opacity:.8;margin-top:0;"><?= $e($t('Sign in to send someone a date invite.')) ?></p>
    <?php if (!empty($error)): ?>
    ...
    <p style="text-align:center;margin:14px 0 6px;opacity:.6;"><?= $e($t('No account yet?')) ?> <a href="/" style="color:#ff3d8b;font-weight:600;"><?= $e($t('Start here')) ?></a></p>
    <details style="margin-top:6px;">
      <summary style="cursor:pointer;opacity:.7;font-size:14px;"><?= $e($t('Other ways to sign in')) ?></summary>
      <form method="post" action="/login/magic" style="display:flex;flex-direction:column;gap:8px;margin-top:8px;">
        ...
        <button type="submit" style="padding:10px;border:0;border-radius:12px;border:1px solid #e7d4ff;background:#fff;color:#5a2a52;font-weight:600;cursor:pointer;"><?= $e($t('Email me a magic link')) ?></button>
      </form>
      <a href="/auth/google" style="display:block;text-align:center;margin-top:8px;padding:10px;border-radius:12px;border:1px solid #e7d4ff;color:#5a2a52;text-decoration:none;font-weight:600;"><?= $e($t('Continue with Google')) ?></a>
    </details>
  <?php endif;
  return ob_get_clean();
};
